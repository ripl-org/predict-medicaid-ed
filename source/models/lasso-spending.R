...
# Resrict training sample to spending > 0
y_train <- y[train, outcome]
emergent <- y_train > 0
X_train <- X_train[emergent,]
y_train <- as.vector(y_train[emergent])
...
models <- lapply(gammas, function(gamma) {
  model <- gamlr(x=X_train, y=y_train, family="gaussian", gamma=gamma, standardize=FALSE)
